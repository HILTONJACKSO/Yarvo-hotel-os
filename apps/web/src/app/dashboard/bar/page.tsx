    ...
    <div className="bar-layout">
      <div className="stats-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '24px' }}>
        <div className="stat-card" style={{ background: 'hsl(222, 35%, 15%)', padding: '20px', borderRadius: '8px', border: '1px solid hsl(217, 20%, 25%)' }}>
          <h3 style={{ color: 'hsl(215, 20%, 65%)', fontSize: '0.875rem', margin: '0 0 8px 0' }}>Bar Orders Today</h3>
          <p style={{ color: 'white', fontSize: '1.5rem', fontWeight: 'bold', margin: 0 }}>{stats?.totalOrders || 0}</p>
        </div>
        <div className="stat-card" style={{ background: 'hsl(222, 35%, 15%)', padding: '20px', borderRadius: '8px', border: '1px solid hsl(217, 20%, 25%)' }}>
          <h3 style={{ color: 'hsl(215, 20%, 65%)', fontSize: '0.875rem', margin: '0 0 8px 0' }}>Bar Revenue Generated</h3>
          <p style={{ color: 'hsl(43,96%,56%)', fontSize: '1.5rem', fontWeight: 'bold', margin: 0 }}>${(stats?.totalRevenue || 0).toFixed(2)}</p>
        </div>
      </div>
      <h2>Bar Management Screen</h2>
      
      <div className="ticket-grid">
        {barOrders.length === 0 && <p className="no-tickets">No active drink orders.</p>}
              ...
              {order.items.map(item => (
                <div key={item.id} className={`t-item status-${item.status.toLowerCase()}`}>
                  ...
                  <div className="t-actions">
                    {item.status === 'PENDING' && (
                      <button onClick={() => updateStatus(item.id, 'COOKING')}>Start Preparing</button>
                    )}
                    {item.status === 'COOKING' && (
      ...
      <style>{`
        ...
        .t-item.status-pending { border-left-color: hsl(280, 84%, 60%); } /* Purple for bar pending */
        .t-item.status-cooking { border-left-color: hsl(200, 96%, 56%); } /* Blue for bar making */
