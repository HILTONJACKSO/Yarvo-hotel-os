...
export function AddRoomTypeModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.message || 'Failed to add room type');
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Room Type">
      <form onSubmit={handleSubmit(onSubmit)} className="add-room-type-form">
        {submitError && (
        ...
        <div className="form-row">
          <div className="form-group">
            <label>Name</label>
            <input type="text" {...register('name')} className={errors.name ? 'error' : ''} placeholder="e.g. Deluxe Ocean View" />
            {errors.name && <span className="error-text">{errors.name.message}</span>}
          </div>

          <div className="form-group">
            <label>Code</label>
            <input type="text" {...register('code')} className={errors.code ? 'error' : ''} placeholder="e.g. DLX-OV" />
        ...
        <div className="form-group">
          ...
          <textarea {...register('description')} rows={3} placeholder="A luxurious room overlooking..." />
        </div>

        <div className="form-actions">
          <button type="button" className="btn-cancel" onClick={onClose} disabled={isSubmitting}>Cancel</button>
          <button type="submit" className="btn-submit" disabled={isSubmitting}>
            {isSubmitting ? <Loader2 className="spinner" size={16} /> : 'Add Room Type'}
          </button>
        </div>
      </form>
